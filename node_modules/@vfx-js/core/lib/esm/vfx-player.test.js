...
describe("parseOverflowOpts", () => {
    ...
    test("object", () => {
        ...
        expect(parseOverflowOpts({ top: 100, right: 200, bottom: 300, left: 400 })).toStrictEqual([
            false,
            {
                top: 100,
                right: 200,
...
describe("isRectInViewport", () => {
    ...
    test("no overflow", () => {
        ...
        // adjacent rects
        expect(isRectInViewport(rect(0, 0, 1, 1), rect(-1, 0, 1, 1))).toBe(true);
        expect(isRectInViewport(rect(0, 0, 1, 1), rect(1, 0, 1, 1))).toBe(true);
        expect(isRectInViewport(rect(0, 0, 1, 1), rect(0, -1, 1, 1))).toBe(true);
        expect(isRectInViewport(rect(0, 0, 1, 1), rect(0, 1, 1, 1))).toBe(true);
        // distant rects
        expect(isRectInViewport(rect(0, 0, 1, 1), rect(-2, 0, 1, 1))).toBe(false);
        expect(isRectInViewport(rect(0, 0, 1, 1), rect(2, 0, 1, 1))).toBe(false);
        expect(isRectInViewport(rect(0, 0, 1, 1), rect(0, -2, 1, 1))).toBe(false);
        expect(isRectInViewport(rect(0, 0, 1, 1), rect(0, 2, 1, 1))).toBe(false);
    });
});
